...
const defaultNamespace = process.env.NAMESPACE || 'default';

class Model extends Entity {
    static compile(kind, schema, gstore, data) {
     ...
     */
    static get(id, ancestors, namespace = defaultNamespace, transaction, options, cb) {
        const _this = this;
    ...
    static update(id, data, ancestors, namespace = defaultNamespace, transaction, options, cb) {
     ...
     */
    static query(namespace = defaultNamespace, transaction) {
        const _this = this;
        const query = initQuery(this, namespace = defaultNamespace, transaction);
    ...
    static delete(id, ancestors, namespace = defaultNamespace, transaction, key, cb) {
        this.__hooksEnabled = true;
    ...
    static deleteAll(ancestors, namespace = defaultNamespace, cb) {
        const _this = this;
        const args = Array.prototype.slice.apply(arguments);
    ...
    static findOne(params, ancestors, namespace = defaultNamespace, cb) {
        this.__hooksEnabled = true;
        ...
        function onSuccess(queryData) {
            ...
            entity.toJSON = () => entity.entityData; // TODO: remove this is something goes wrong
    ...
    static findById(id, ancestors, namespace = defaultNamespace, cb) {
        this.__hooksEnabled = true;
        ...
        if (!is.string(id)) {
            return cb({
                code: 400,
                message: 'The id need to be a string',
            });
        }
    ...
    static findAround(property, value, options, namespace = defaultNamespace, cb) {
        const _this = this;
        const args = Array.prototype.slice.apply(arguments);
        cb = args.pop();
     ...
     * @private
     */
    static __model(data, id, ancestors, namespace = defaultNamespace, key) {
        const M = this.compile(this.entityKind, this.schema, this.gstore, data);
    ...
    static __scopeHook(hook, args) {
        ...
        function deleteScope() {
            ...
            return multiple ? null : _this.__model(null, id, ancestors, namespace = defaultNamespace, key);
        }
    }
    ...
    save(transaction, options, cb) {
        ...
        function prepareData() {
            ...
            function _id() {
              if (!this.entityData._id) this.entityData._id = crypto.randomBytes(20).toString('hex');
            }
            function createdAt() {
              if (!this.entityData.createdAt) this.entityData.createdAt = new Date();
            }
            ...
            */
            function updateModifiedOn() {
                ...
                // }
                this.entityData.updatedAt = new Date();
            }
    ...
    validate() {
        ...
        Object.keys(this.entityData).forEach((k) => {
            ...
            if (k === 'createdAt' || k === 'updatedAt' || k === '_id') return { success: true };
...
function initQuery(self, namespace = defaultNamespace, transaction) {
    if (transaction && transaction.constructor.name !== 'Transaction') {
        throw Error('Transaction needs to be a gcloud Transaction');
    }
